...
**[缘起](#为什么需要这个项目)** · **[原则](#production-first-原则)** · **[自主执行](#自主且克制地执行)** · **[规则手册](#选择规则手册)** · **[安全门](#安全门)** · **[并行协作](#为并行协作而设计)** · **[快速开始](#快速开始)**
...
- 检查仓库状态，自行解决可发现的事实，在任务范围内作出合理假设并继续执行；只有当缺失信息会实质改变范围、用户可见行为、数据或授权安全，或需要外部操作或破坏性操作时，才提出询问。
- 实现最小但完整的变更；不得加入推测性功能、抽象、配置、重构、格式调整或无关清理。只删除因当前变更而不再使用的代码。
- 对陌生或非简单能力，应先检查项目现有模式，再独立查阅官方文档以及信誉良好、持续维护的 GitHub 实现。验证版本兼容性、许可证、安全性和维护状态；GitHub 仅是参考，不代表可以自动安装依赖或整套复制实现。对于项目已有成熟模式的常规变更，应跳过外部研究。
